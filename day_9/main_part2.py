    ...
    def attract(self, other):
        ...
        if d <= 1:
            ...
            if abs(self._x - other._x) == abs(self._y - other._y):
                return Pos(self._x + sign(other._x - self._x), self._y + sign(other._y - self._y))
            elif abs(self._x - other._x) < abs(self._y - other._y):
...
with open("input") as f:
    for l in f.readlines():
        ...
        while count > 0:
            ...
            previousKnotPos = headPos
            for k in range(9):
                knotsPos[k] = previousKnotPos.attract(knotsPos[k])
                previousKnotPos = knotsPos[k]
            visitedTailPos.add(knotsPos[8])
            count -= 1
